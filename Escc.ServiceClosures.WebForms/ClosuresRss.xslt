...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
 ...
 >
  ...
  <xsl:template match="/">
    ...
    <xsl:value-of select="$HtmlXsltUrl"/>
    <xsl:text disable-output-escaping="yes">" ?&gt;
        &lt;?xml-stylesheet  type="text/css" href="</xsl:text>
  ...
  <xsl:template match="closures:Service">
    ...
    <xsl:for-each select="closures:Closures/closures:Closure[number(concat(substring(closures:EndDate,1,4), substring(closures:EndDate,6,2), substring(closures:EndDate,9,2)))>=concat(substring($Iso8601Date,1,4), substring($Iso8601Date,6,2), substring($Iso8601Date,9,2))]">
      ...
      <!-- Where there is a known two word status, display it with correct spacing -->
      <xsl:variable name="Status">
        <xsl:choose>
          <xsl:when test="closures:Status='PartlyClosed'">
            <xsl:text>Partly closed</xsl:text>
          </xsl:when>
          <xsl:otherwise>
      ...
      <item>
        ...
        <title>
          ...
          <xsl:text> </xsl:text>
          <xsl:value-of select="translate($Status,$Uppercase,$Lowercase)"/>
          <xsl:text>, </xsl:text>
        ...
        <xsl:choose>
          <xsl:when test="closures:DateModified != '0001-01-01T00:00:00'">
            <pubDate>
              <xsl:call-template name="FormatRFC822Date">
        ...
        <guid isPermaLink="false">
          <xsl:value-of select="closures:LinkedDataUri"/>
        </guid>
